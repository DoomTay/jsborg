...
localFileLoader.prototype =
{
	...
	load: function(url, onLoad, onProgress, onError)
	{
		...
		reader.addEventListener("load", function(e)
		{
			...
			{
				var fileLoader = new THREE.CWCTRLLoader(this.manager);
				fileLoader.load(reader.result,onLoad, onProgress, onError);
			}
			else if(path.endsWith(".url"))
			{
				...
				fileLoader.load(reader.result,function(data)
				{
					...
					var fullBorgPath = new URL(file.webkitRelativePath,"C:\fakepath").href;
					
					console.log(fullBorgPath);
					
					path = new URL(path,fullBorgPath + "domains/");
				}, onProgress, onError);
			}
			else
			{
